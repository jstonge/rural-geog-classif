...
which parsers (docling, olmocr, dots) have produced output for it,
whether it has been summarized, and whether it matched an OpenAlex record.
...
def main():
    ...
    # Collect existing files as sets for fast lookup. olmocr writes to
    # parse/output/olmocr/<doi>.md after the Makefile post-step flattens the
    # markdown/ subdir, but pre-flatten files may still be at olmocr/markdown/.
    ...
    if OLMOCR_DIR.exists():
        olmocr_stems = {p.stem for p in OLMOCR_DIR.glob("*.md")}
        olmocr_stems |= {p.stem for p in (OLMOCR_DIR / "markdown").glob("*.md")}
    # dots.ocr: flattened <doi>.md at top level (post-flatten), or per-page
    # *_nohf.md files in <doi>/ (parsing completed but flatten not yet run).
    dots_stems = set()
    if DOTS_DIR.exists():
    ...
    counts = {"total": 0, "has_pdf": 0, "docling_parsed": 0,
              "olmocr_parsed": 0, "dots_parsed": 0,
              "any_parsed": 0, "all_parsed": 0,
    ...
    for row in rows:
        ...
        olmocr_parsed = key in olmocr_stems
        dots_parsed = key in dots_stems
        ...
        counts["olmocr_parsed"] += olmocr_parsed
        counts["dots_parsed"] += dots_parsed
        counts["any_parsed"] += docling_parsed or olmocr_parsed or dots_parsed
        counts["all_parsed"] += docling_parsed and olmocr_parsed and dots_parsed
    ...
    for key in ["has_pdf", "docling_parsed", "olmocr_parsed", "dots_parsed",
                "any_parsed", "all_parsed", "summarized", "match_oa"]:
        n = counts[key]
